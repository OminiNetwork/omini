...
import (
	...
	"github.com/omini/omini/v20/app/eips/testdata"
...
func TestIPs(t *testing.T) {
	...
	RunSpecs(t, "ominiIPs Suite")
}

var _ = Describe("Improvement proposal omini_0 - ", Ordered, func() {
	var (
		in network.Network
	...
	// Multiplier used to modify the opcodes associated with omini_0 IP.
	...
	It("should enable the new IP", func() {
		...
		qRes, err = gh.GetEvmParams()
		Expect(err).To(BeNil(), "failed during query to evm params")
		Expect(qRes.Params.ExtraEIPs).To(ContainElement(newIP), "expected to have IP omini_0 in evm params")
	})

	It("should change CREATE opcode constant gas after enabling omini_0 IP", func() {
...
var _ = Describe("Improvement proposal omini_1 - ", Ordered, func() {
	var (
		in network.Network
		tf factory.TxFactory
		gh grpc.Handler
	...
	// Multiplier used to modify the opcodes associated with omini_1.
	...
	It("should enable the new IP", func() {
		...
		qRes, err = gh.GetEvmParams()
		Expect(err).To(BeNil(), "failed during query to evm params")
		Expect(qRes.Params.ExtraEIPs).To(ContainElement(newIP), "expected to have ip omini_1 in evm params")
	})
...
var _ = Describe("Improvement proposal omini_2 - ", Ordered, func() {
	var (
		in network.Network
		tf factory.TxFactory
		gh grpc.Handler
	...
	// Constant gas used to modify the opcodes associated with omini_2.
	...
	It("should enable the new IP", func() {
		...
		qRes, err = gh.GetEvmParams()
		Expect(err).To(BeNil(), "failed during query to evm params")
		Expect(qRes.Params.ExtraEIPs).To(ContainElement(newIP), "expected to have ip omini_2 in evm params")
	})
