...
from .network import omini, create_snapshots_dir, setup_custom_omini
from .utils import (
    CONTRACTS,
    omini_6DEC_CHAIN_ID,
    decode_bech32,
...
def custom_omini_6dec(tmp_path_factory):
    ...
    yield from setup_custom_omini(
        path, 46810, evm6dec_config(path, "rollback-test"), chain_id=omini_6DEC_CHAIN_ID
    )
...
def grpc_eth_call(port: int, args: dict, chain_id=None, proposer_address=None):
    ...
    return requests.get(f"http://localhost:{port}/omini/evm/v1/eth_call", params).json()


def test_grpc_mode(omini_cluster: omini):
    """
    ...
    api_port = ports.api_port(omini_cluster.base_port(1))
    ...
    for i in range(2):
        wait_for_block(omini_cluster.cosmos_cli(i), 1)
    ...
    with open(omini_cluster.base_dir / "node1.log", "a", encoding="utf-8") as logfile:
        proc = subprocess.Popen(  # pylint: disable=consider-using-with
            [
                omini_cluster.chain_binary,
                "start",
                "--grpc-only",
                "--home",
                omini_cluster.base_dir / "node1",
            ],
            stdout=logfile,
        ...
        try:
            ...
            addr = omini_cluster.cosmos_cli(0).consensus_address()
            ...
            rsp = grpc_eth_call(
                api_port,
                msg,
                chain_id=omini_cluster.cosmos_cli().chain_id,
