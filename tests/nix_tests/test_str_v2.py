...
# uatom from cosmoshub-2 -> cosmoshub-1 IBC representation on the omini chain.
...
# The ERC20 address of ATOM on omini
...
UPDATE_PARAMS_PROP = {
    "messages": [
        {
            "@type": "/omini.erc20.v1.MsgUpdateParams",
...
def ibc(request, tmp_path_factory):
    ...
    network = prepare_network(path, name, [omini_build, "cosmoshub-1", "cosmoshub-2"])
    yield from network
...
def test_str_v2_single_hop(ibc):
    ...
    old_dst_balance = get_balance(omini, bech_dst, ATOM_IBC_DENOM)
    ...
    new_dest_balance = get_balance(omini, bech_dst, ATOM_IBC_DENOM)
    erc_dest_balance = erc20_balance(w3, ATOM_1_ERC20_ADDRESS, omini_addr)
...
def test_str_v2_multi_hop(ibc):
    ...
    omini_old_balance = get_balance(omini, bech_dst, ATOM_2_IBC_DENOM_MULTI_HOP)
    ...
    omini_balance = get_balance(omini, bech_dst, ATOM_2_IBC_DENOM_MULTI_HOP)
    dynamic_precompiles = omini_cli.erc20_params()["params"]["dynamic_precompiles"]
    token_pairs = omini_cli.get_token_pairs()

    # Here it's only one from the previous one we've registered in the first test
    assert omini_old_balance + 50000 == omini_balance
    assert len(dynamic_precompiles) == 1
    assert dynamic_precompiles[0] == ATOM_1_ERC20_ADDRESS
    assert len(token_pairs) == 2


def test_womini_precompile_transfer(ibc):
    """
    Test the ERC20 transfer from one signer to another using the now
    registered ERC20 precompiled contract for Womini.
    """
    assert_ready(ibc)
    ...
    w3 = omini.w3
    omini_balance = get_balance(omini, bech_dst, src_denom)
    signer2_balance = erc20_balance(w3, Womini_ADDRESS, signer2)

    assert omini_balance == signer2_balance

    receipt = erc20_transfer(
        w3, Womini_ADDRESS, signer1, signer2, 1000000, KEYS["signer1"]
    )
    assert receipt.status == 1

    signer_2_balance_after = erc20_balance(w3, Womini_ADDRESS, signer2)
    assert signer_2_balance_after == signer2_balance + 1000000

    omini_balance_after = get_balance(omini, bech_dst, src_denom)
    assert omini_balance_after == omini_balance + 1000000
...
def test_toggle_erc20_precompile(ibc):
    ...
    acc = omini_cli.evm_account(contract_bech32_addr)
...
def update_erc20_params(omini: omini, native_precomiles=[], dynamic_precompiles=[]):
