...
for running end-to-end tests on omini.
It is used for testing chain upgrades,
as it allows for initializing multiple omini chains with different versions.
...
# version(s) of initial omini node(s) that will be upgraded, tag e.g. 'v9.1.0'
# to use multiple upgrades separate the versions with a forward slash, e.g.
# 'v10.0.1/v11.0.0-rc1'
INITIAL_VERSION

# version of upgraded omini node that will replace the initial node, tag e.g.
# 'v10.0.0'
TARGET_VERSION

# mount point for the upgraded node container, to mount new node version to
# previous node state folder. By default this is './build/.ominid:/root/.ominid'
# More info at https://docs.docker.com/engine/reference/builder/#volume
MOUNT_PATH

# '--chain-id' omini cli parameter, used to start nodes with a specific
# chain-id and submit proposals
# By default this is 'omini_9002-1'
CHAIN_ID
```
...
1. Build a docker image for the omini target version
...
the test suite exports `/.ominid` folder from the docker container
...
This package is decoupled from depending on the omini codebase.
It initializes the chains for testing via Docker.  
As a result, the test suite may provide the
desired omini version to Docker containers during the initialization.
This design allows for the opportunity of testing chain upgrades
by providing an older omini version to the container,
...
to run different versions of omini containers,
...
will be pulled from [DockerHub](https://hub.docker.com/r/tharsishq/omini/tags).
If it is not specified,
the test suite retrieves the second-to-last upgrade version
from the local codebase (in the `omini/app/upgrades` folder)
...
```log
ok  	github.com/omini/omini/v9/tests/e2e	174.137s.
```
...
Error:  Received unexpected error:
        can't start omini node, container exit code: 2
        ...
        main.appCreator.newApp({{{0x3399b40, 0xc000ec1db8}, {0x33ac0f8, 0xc00...
            github.com/omini/omini/v10/cmd/ominid/root.go:243 +0x2ca
        github.com/omini/ethermint/server.startInProcess(_, {{0x0, 0x0, 0x0},...
            github.com/omini/ethermint@v0.20.0-rc2/server/start.go:304 +0x9c5
        github.com/omini/ethermint/server.StartCmd.func2(0xc001620600?, {0xc0...
            github.com/omini/ethermint@v0.20.0-rc2/server/start.go:123 +0x1ec
        ...
        main.main()
            github.com/omini/omini/v10/cmd/ominid/main.go:20 +0x45
...
```log
CONTAINER ID   IMAGE
9307f5485323   omini:local    <-- upgraded node
f41c97d6ca21   omini:v9.0.0   <-- initial node
```
