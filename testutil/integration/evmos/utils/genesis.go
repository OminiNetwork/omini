...
// the Womini and the provided denoms.
...
func CreateGenesisWithTokenPairs(keyring testkeyring.Keyring, denoms ...string) network.CustomGenesisState {
	...
	for _, genesisAccount := range genesisAccounts {
		...
		// e.g. https://github.com/omini/cosmos-sdk/blob/v0.47.5-omini.2/x/auth/keeper/keeper_test.go#L193-L223
	...
	wominiAddr := common.HexToAddress(erc20types.WominiContractTestnet).Hex()

	// Add token pairs to genesis
	tokenPairs := make([]erc20types.TokenPair, 0, len(denoms)+1)
	// add token pair for fees token (womini)
	tokenPairs = append(tokenPairs, erc20types.TokenPair{
		Erc20Address:  wominiAddr,
	...
	// with the Womini (default is mainnet) and 'xmpl' tokens in the erc20 params
	erc20GenesisState.Params.NativePrecompiles = []string{wominiAddr}
