...
import (
	...
	ominiibc "github.com/omini/omini/v20/ibc/testing"
	"github.com/omini/omini/v20/testutil/integration/common/network"
	ibcchain "github.com/omini/omini/v20/testutil/integration/ibc/chain"
)
...
type Coordinator interface {
	...
	GetPath(a, b string) *ominiibc.Path
...
// GetPath returns the transfer path for the chain ids 'a' and 'b'
func (c *IntegrationCoordinator) GetPath(a, b string) *ominiibc.Path {
	chainA := c.coord.GetChain(a)
	chainB := c.coord.GetChain(b)

	return ominiibc.NewTransferPath(chainA, chainB)
}
...
func (c *IntegrationCoordinator) Setup(a, b string) IBCConnection {
	path := c.GetPath(a, b)
	ominiibc.SetupPath(c.coord, path)
