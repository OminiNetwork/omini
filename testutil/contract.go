...
import (
	...
	evm "github.com/omini/omini/v20/x/evm/types"
)
...
type ContractCallArgs struct {
	...
	Nonce *big.Int
	// Amount is the aomini amount to send in the transaction.
	Amount *big.Int
...
func DeployContract(
	...
	nonce := ominiApp.EvmKeeper.GetNonce(ctx, from)
	...
	baseFeeRes, err := ominiApp.EvmKeeper.BaseFee(ctx, &evm.QueryBaseFeeRequest{})
	if err != nil {
		return common.Address{}, err
	}
	...
	res, err := DeliverEthTx(ominiApp, priv, msgEthereumTx)
	if err != nil {
		return common.Address{}, err
	}

	if _, err := CheckEthTxResponse(res, ominiApp.AppCodec()); err != nil {
		return common.Address{}, err
	}
...
func DeployContractWithFactory(
	...
	chainID := evm.GetEthChainConfig().ChainID
	from := common.BytesToAddress(priv.PubKey().Address().Bytes())
	factoryNonce := ominiApp.EvmKeeper.GetNonce(ctx, factoryAddress)
	nonce := ominiApp.EvmKeeper.GetNonce(ctx, from)
