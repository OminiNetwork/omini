...
var (
	...
	DefaultFee = sdk.NewCoin(ominitypes.BaseDenom, sdkmath.NewIntFromUint64(uint64(feeAmt))) // 0.01 omini
)
...
type CosmosTxArgs struct {
	...
	// ChainID is the chain's id on cosmos format, e.g. 'omini_9000-1'
	ChainID string
...
func PrepareCosmosTx(
	...
	if args.GasPrice != nil {
		fees = sdk.Coins{{Denom: ominitypes.BaseDenom, Amount: args.GasPrice.MulRaw(int64(args.Gas))}} //#nosec G115
	} else {
...
func signCosmosTx(
	...
	seq, err := appomini.AccountKeeper.GetSequence(ctx, addr)
	if err != nil {
		return nil, err
	}
	...
	accNumber := appomini.AccountKeeper.GetAccount(ctx, addr).GetAccountNumber()
