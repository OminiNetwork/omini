	...
	testCases := []struct {
		...
		},
		{
			...
			map[string]func(*JumpTable){
				"omini_0": func(jt *JumpTable) {},
			},
			true,
			"",
			func() {
				eips := GetActivatorsEipNames()
				require.ElementsMatch(t, append(eips_snapshot, "omini_0"), eips, "expected eips number to be equal")
			},
		},
		{
			"success - multiple new activators",
			map[string]func(*JumpTable){
				"omini_1": func(jt *JumpTable) {},
				"omini_2": func(jt *JumpTable) {},
			},
			true,
			"",
			func() {
				...
				require.ElementsMatch(t, append(eips_snapshot, "omini_0", "omini_1", "omini_2"), eips, "expected eips number to be equal")
			},
		},
		{
