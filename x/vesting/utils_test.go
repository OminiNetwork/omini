...
// SetupWithGenesisValSet initializes a new ominiApp with a validator set and genesis accounts
...
func (s *VestingTestSuite) SetupWithGenesisValSet(valSet *cmttypes.ValidatorSet, genAccs []authtypes.GenesisAccount, balances ...banktypes.Balance) {
	appI, genesisState := ominiapp.SetupTestingApp(cmn.DefaultChainID)()
	app, ok := appI.(*ominiapp.omini)
	s.Require().True(ok)
	...
	bondAmt := sdk.TokensFromConsensusPower(1, ominitypes.PowerReduction)
	...
	for _, b := range balances {
		...
		totalSupply = totalSupply.Add(b.Coins.Add(sdk.NewCoin(ominitypes.BaseDenom, totalBondAmt))...)
	}
	...
	_, err = app.InitChain(
		&abci.RequestInitChain{
			...
			ConsensusParams: ominiapp.DefaultConsensusParams,
...
func (s *VestingTestSuite) DoSetupTest() {
	...
	amount := sdk.TokensFromConsensusPower(5, ominitypes.PowerReduction)
