...
func TestGetominiAddressFromBech32(t *testing.T) {
	testCases := []struct {
		name       string
		address    string
		expAddress string
		...
		{
			"omini address",
			"omini1qql8ag4cluz6r4dz28p3w00dnc9w8ueuafmxps",
			"omini1qql8ag4cluz6r4dz28p3w00dnc9w8ueuafmxps",
			false,
		},
		{
	...
	for _, tc := range testCases {
		addr, err := GetominiAddressFromBech32(tc.address)
		if tc.expError {
...
func TestominiCoinDenom(t *testing.T) {
	testCases := []struct {
		name     string
		denom    string
		expError bool
...
func TestAddressConversion(t *testing.T) {
	...
	bech32 := "omini10jmp6sgh4cc6zt3e8gw05wavvejgr5pwjnpcky"
