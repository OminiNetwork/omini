...
HTTPS_GIT := https://github.com/omini/omini.git
...
# Deps
DEPS_COSMOS_SDK_VERSION := $(shell cat go.sum | grep -E 'github.com/omini/cosmos-sdk\s' | grep -v -e 'go.mod' | tail -n 1 | awk '{ print $$2; }')
...
ldflags = -X github.com/cosmos/cosmos-sdk/version.Name=omini \
          -X github.com/cosmos/cosmos-sdk/version.AppName=$(omini_BINARY) \
...
build-docker-goleveldb:
	...
	mkdir -p ./build/.ominid
	echo '#!/usr/bin/env bash' > ./build/ominid
	echo "IMAGE_NAME=${DOCKER_IMAGE}:${COMMIT_HASH}" >> ./build/ominid
	echo 'SCRIPT_PATH=$$(cd $$(dirname $$0) && pwd -P)' >> ./build/ominid
	echo 'docker run -it --rm -v $${SCRIPT_PATH}/.ominid:/home/omini/.ominid $$IMAGE_NAME ominid "$$@"' >> ./build/ominid
	chmod +x ./build/ominid
...
godocs:
	@echo "--> Wait a few seconds and visit http://localhost:6060/pkg/github.com/omini/omini"
	godoc -http=:6060
...
proto-download-deps:
	...
	git init && \
	git remote add origin "https://github.com/omini/cosmos-sdk.git" && \
...
PACKAGE_NAME:=github.com/omini/omini
...
# omini repository and then clean up the contracts data.
...
# Compile the solidity contracts found in the omini repository.
contracts-compile:
