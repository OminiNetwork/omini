...
const DefaultFeeAmt = int64(150_000_000_000_000_000) // 0.15 omini

var globalStartTime = time.Date(2020, 1, 2, 0, 0, 0, 0, time.UTC)

// NewCoordinator initializes Coordinator with N EVM TestChain's (omini apps) and M Cosmos chains (Simulation Apps)
...
func SendMsgs(chain *ibctesting.TestChain, feeAmt int64, msgs ...sdk.Msg) (*sdk.Result, error) {
	...
	if ominiChain, ok := chain.App.(*app.omini); ok {
		bondDenom, err = ominiChain.StakingKeeper.BondDenom(chain.GetContext())
	} else {
