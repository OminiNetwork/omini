...
import (
	...
	ominikr "github.com/omini/omini/v20/crypto/keyring"
	"github.com/omini/omini/v20/server/config"
	srvflags "github.com/omini/omini/v20/server/flags"
	"github.com/omini/omini/v20/testutil/network"
...
func addTestnetFlagsToCmd(cmd *cobra.Command) {
	...
	cmd.Flags().String(sdkserver.FlagMinGasPrices, fmt.Sprintf("0.000006%s", ominitypes.BaseDenom), "Minimum gas prices to accept for transactions; All fees in a tx must meet this minimum (e.g. 0.01photino,0.001stake)")
...
Example:
	ominid testnet init-files --v 4 --output-dir ./.testnets --starting-ip-address 192.168.10.2
	`,
	...
	cmd.Flags().String(flagNodeDaemonHome, "ominid", "Home directory of the node's daemon configuration")
...
Example:
	ominid testnet --v 4 --output-dir ./.testnets
	`,
		RunE: func(cmd *cobra.Command, _ []string) error {
			args := startArgs{}
			args.outputDir, _ = cmd.Flags().GetString(flagOutputDir)
...
func initTestnetFiles(
	...
	if args.chainID == "" {
		args.chainID = fmt.Sprintf("omini_%d-1", cmtrand.Int63n(9999999999999)+1)
	}
	...
	for i := 0; i < args.numValidators; i++ {
		...
		kb, err := keyring.New(sdk.KeyringServiceName(), args.keyringBackend, nodeDir, inBuf, clientCtx.Codec, ominikr.Option())
		if err != nil {
			return err
		}
		...
		accStakingTokens := sdk.TokensFromConsensusPower(5000, ominitypes.PowerReduction)
		coins := sdk.Coins{
			sdk.NewCoin(ominitypes.BaseDenom, accStakingTokens),
		}

		genBalances = append(genBalances, banktypes.Balance{Address: addr.String(), Coins: coins.Sort()})
		genAccounts = append(genAccounts, authtypes.NewBaseAccount(addr, nil, 0, 0))

		valTokens := sdk.TokensFromConsensusPower(100, ominitypes.PowerReduction)
		createValMsg, err := stakingtypes.NewMsgCreateValidator(
			sdk.ValAddress(addr).String(),
			valPubKeys[i],
			sdk.NewCoin(ominitypes.BaseDenom, valTokens),
		...
		txBuilder.SetFeeAmount(sdk.NewCoins(sdk.NewCoin(ominitypes.BaseDenom, minGasPrice.MulInt64(createValidatorMsgGasLimit).Ceil().TruncateInt())))
		...
		customAppTemplate, customAppConfig := config.AppConfig(ominitypes.BaseDenom)
	...
	if err := initGenFiles(clientCtx, mbm, args.chainID, ominitypes.BaseDenom, genAccounts, genBalances, genFiles, args.numValidators, args.baseFee, args.minGasPrice); err != nil {
		return err
	}
